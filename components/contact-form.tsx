...
export default function ContactForm({initialService=''}:{initialService?:string}) {
  ...
  async function submit(event:React.FormEvent<HTMLFormElement>){
    ...
    try {
      await submitInquiry({...Object.fromEntries(data),service:selected},{url:process.env.NEXT_PUBLIC_SUPABASE_URL||config.url,anonKey:process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY||config.anonKey},services.find(s=>s.slug===selected)?.title||'기타');
      setState('success');form.current?.reset();
  ...
  return <form ref={form} onSubmit={submit} className="inquiry-form">
    <fieldset><legend><span>01</span> 어떤 도움이 필요하신가요? <small>선택</small></legend><div className="service-choices">{[...services.map(s=>({slug:s.slug,title:s.title})),{slug:'not-sure',title:'아직 정하지 못했어요'}].map(s=><button type="button" key={s.slug} aria-pressed={selected===s.slug} onClick={()=>setSelected(selected===s.slug?'':s.slug)}>{s.title}</button>)}</div></fieldset>
    <fieldset><legend><span>02</span> 회신받을 연락처를 남겨주세요.</legend><div className="form-grid"><label>이름 / 브랜드명 <span>*</span><input name="name" autoComplete="organization" placeholder="담당자 또는 브랜드 이름" required maxLength={120}/></label><label>이메일 <span>*</span><input name="email" type="email" autoComplete="email" placeholder="hello@yourbrand.com" required maxLength={254}/></label><label>연락처 <span>*</span><input name="phone" type="tel" autoComplete="tel" placeholder="010-0000-0000" required pattern={"[0-9+\\s\\-\\(\\).]{7,30}"} maxLength={30}/></label><label>예상 예산 <small>선택</small><select name="budget" defaultValue=""><option value="">아직 정해지지 않았어요</option><option>1,000만원 미만</option><option>1,000–3,000만원</option><option>3,000–5,000만원</option><option>5,000만원–1억원</option><option>1억원 이상</option></select></label></div></fieldset>
