...
export default function Hero({projects,uncropped=false}:{projects:HeroProject[];uncropped?:boolean}) {
 ...
 <button className="hero-zone hero-prev cursor-prev" aria-label="이전 프로젝트" onClick={()=>{move(-1);setPaused(true);}}><span className="hero-zone-label" aria-hidden="true">PREV</span></button><button className="hero-zone hero-next cursor-next" aria-label="다음 프로젝트" onClick={()=>{move(1);setPaused(true);}}><span className="hero-zone-label" aria-hidden="true">NEXT</span></button>
