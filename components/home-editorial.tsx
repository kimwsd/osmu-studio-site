...
export default function HomeEditorial({section}:{section:'ambition'|'stories'}){
  ...
  return <section className="inside-osmu section-pad"><h2 className="section-rule">{content.insideTitle}</h2><div className="story-grid">{content.stories.map(story=><article className="story-card" key={story.href}><Link href={story.href} className="story-image"><img src={`/assets/work-generated/v2/${story.image}-02.webp`} alt={story.title} width="800" height="900" loading="lazy"/><span className="read-more-strip" aria-hidden="true"><span>READ MORE　READ MORE　READ MORE　</span><span>READ MORE　READ MORE　READ MORE　</span></span></Link><span className="story-category">{story.label}</span><h3><Link href={story.href}>{story.title}</Link></h3><p>{story.copy}</p><Link className="text-link" href={story.href}><ArrowRight size={16}/>Full story</Link></article>)}</div></section>;
}
