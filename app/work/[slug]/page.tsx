...
export default async function ProjectPage({params}:Props){const {slug}=await params,p=await getProject(slug);if(!p)notFound();const all=await getProjects(),index=all.findIndex(item=>item.slug===slug),next=all[(index+1)%all.length];return <article className="project-page">
 ...
 <section className="project-story section-pad"><div className="project-title"><h1>{p.name}</h1></div><ProjectNarrative summary={p.summary||'브랜드의 생각을 형태로.'} body={p.body||'프로젝트의 상세한 작업 내용은 문의를 통해 안내해 드립니다.'}/><div className="project-facts"><dl><dt>Discipline</dt><dd>{p.cat}</dd><dt>Year</dt><dd>{p.year}</dd><dt>Project type</dt><dd>{p.concept?'Concept Project / 가상 브랜드':p.loc||'—'}</dd></dl>{p.concept&&<p className="concept-note">서비스의 시각적 방향을 탐색한 자체 콘셉트 작업입니다. 실제 고객 프로젝트나 사업 성과를 나타내지 않습니다.</p>}{p.images[0]?.startsWith('/')&&<a className="pill-button" href={p.images[0]} download>Download project image</a>}</div></section>
